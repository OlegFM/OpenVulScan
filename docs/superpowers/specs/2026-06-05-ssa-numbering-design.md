...
| CaptureId | Unified `SsaId`, вид `Capture` | Один API для правил. Roslyn `IFlowCaptureOperation` уже даёт SSA-нумерацию temps — оборачиваем без перенумерации. |
...
`IFlowCaptureOperation` создаёт SSA-temp с `CaptureId`. Версия всегда 0, поскольку Roslyn гарантирует SSA-форму для captures (одно def на capture). `IFlowCaptureReferenceOperation` — use этого capture.
